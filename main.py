    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mouse_pos = event.pos
                return_button_rect = pygame.Rect(MAP_WIDTH + 50, HEIGHT // 2 + 25, 100, 50)
                if return_button_rect.collidepoint(mouse_pos):
                    main()

        game_map = scale_image(original_map, MAP_WIDTH, HEIGHT)

        # Update car actions based on neural network output
        ...
        pygame.draw.rect(screen, (200, 200, 200), (MAP_WIDTH, 0, UI_WIDTH, HEIGHT))
            ...
            if car.is_alive():
                ...
                if i == best_car_index:
                    for radar in car.radars:
                        position = radar[0]
                        pygame.draw.line(screen, (255, 255, 0), car.center, position, 1)
                        pygame.draw.circle(screen, (255, 255, 0), position, 5)
                    pygame.draw.rect(screen, (0, 255, 0), (*car.position, CAR_SIZE_X, CAR_SIZE_Y), 2)
...
def preview(screen, original_map, map_info):
    """
    Display a preview of the map and allow the user to start the simulation.
    """
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mouse_pos = event.pos
                next_button_rect = pygame.Rect(MAP_WIDTH + 50, HEIGHT // 2 - 75, 100, 50)
                play_button_rect = pygame.Rect(MAP_WIDTH + 50, HEIGHT // 2 - 25, 100, 50)
                if next_button_rect.collidepoint(mouse_pos):
                    current_map_index = (current_map_index + 1) % len(map_info)
                    MAP_NAME, CAR_SIZE_X, CAR_SIZE_Y, CAR_POS_X, CAR_POS_Y = map_info[current_map_index]
...
def main():
    """
    Main function to initialize and run the NEAT simulation.
    """
    ...
    map_info = load_map_info()  # Load map information from the utils module
    if map_info:
    ...
    while True:
        ...
        if action == 'play':
            ...
            while True:
                result = population.run(lambda genomes, config: run_simulation(genomes, config, screen, original_map), 1)
                if result == 'menu':
                    break
